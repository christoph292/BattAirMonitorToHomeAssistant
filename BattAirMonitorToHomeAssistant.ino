...
#if DEBUG_MODE
  #define TIME_TO_SLEEP  60 //1min/       /* Time ESP32 will go to sleep (in seconds) */
#else
  #define TIME_TO_SLEEP  86400//3600//60// 1 Tag      /* Time ESP32 will go to sleep (in seconds) */
#endif
//Funktionen
...
bool readOutClient(NimBLEClient* pClient, int actualBattery, String deviceAddress){
        ...
        //Mac Adresse in umgekehrter Reihenfolge ins WriteRequest array schreiben
...
void setup() {
 ...
 Serial.println(optionSleep);
  ...
  //Batteriereihenfolge aus Speicher auslesen
  ...
  if (optionSleep !=2 ) pNimBLEScan->start(60, scanCompleteCallback);  // Scan für 60 Sekunden
  else pNimBLEScan->start(30, scanCompleteCallback);  // Scan für 30 Sekunden
}

void loop() {
  if (readOutComplete==true /*|| countLoop > 0*/){
  ...
  if (scanComplete) {
    ...
    for (auto& deviceAddress : foundDevices) {
      ...
      Serial.print("Reihenfolge Position: ");
      ...
      BattAirHandlerArray[actualBattery].Status = foundDevicesRssi[i] + "|" + 0;

      if (optionSleep != 2){
        int attempts = 0; 
        bool connected = false;
        //NimBLEClient* pClient;
        while (attempts < CONNECTIONATTEMPTS && !connected) { 
          Serial.print("Vebindungsversuch: ");
          Serial.println(attempts + 1); 
          pClients[actualBattery] = NimBLEDevice::createClient();
          if (pClients[actualBattery]->connect(NimBLEAddress(deviceAddress.c_str()))){
            BattAirHandlerArray[actualBattery].Status = String(foundDevicesRssi[i]) + "|" + (attempts + 1);
            Serial.print("Status: ");
            Serial.println(BattAirHandlerArray[actualBattery].Status);
            connected = readOutClient(pClients[actualBattery],actualBattery,deviceAddress);
          } else {
          Serial.println("Verbindung zum BattAir Modul fehlgeschlagen.");
          NimBLEDevice::deleteClient(pClients[actualBattery]);
          }
            attempts++; 
  ...
  if(readOutComplete){
    ...
    //for(int i=(countLoop*countLength); i<((countLoop+1)*countLength);i++){
    for(int i=0; i<30;i++){
